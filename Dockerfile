...
# then log output of file /code/bin/facud
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/facud \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/facud | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/facud /usr/bin/facud
