...
HTTPS_GIT := github.com/facundomedica/consumer.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=consumer \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=facud \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/facud ./cmd/facud
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/facud.exe ./cmd/facud
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/facud
...
draw-deps:
	...
	@goviz -i ./cmd/facud -d 2 | dot -Tpng -o dependency-graph.png
...
set-testnet-configs:
	facud config set client chain-id localchain-1
	facud config set client keyring-backend test
	facud config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | facud keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | facud keys add acc1 --recover`
